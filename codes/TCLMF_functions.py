...
def read_inputs(folder):
    ...
    with open(os.path.join(folder, "GDSC_class_median.txt"), "r") as raw:
        
        observation_mat = [line.strip("\n").split() for line in raw]
        
    with open(os.path.join(folder, "drug_similarity_23.txt"), "r") as raw:
        raw.next()
        
        drug_matt = [line.strip("\n").split()[1:] for line in raw]

    with open(os.path.join(folder, "similarity_EXP_980_1.txt"), "r") as raw:
    ...
    with open(os.path.join(folder, "similarity_tissue_Testis.txt"), "r") as raw:
        
        cell_sim_tissue = [line.strip("\n").split()[0:] for line in raw]      
        
        
    with open(os.path.join(folder, "Real_Ic50.txt"), "r") as raww:
        raww.next()        
...
class LMF:
    ...
    def __init__(self, c=1, K1=5, K2=20, r=95, theta=1.3, lambda_p=0.6, lambda_l=0.6, alpha=0.4,beta=0.4, max_iter=50):
        ...
        self.K1 = int(K1) # number of nearest neighbors 
    ...
    def construct_neighborhood(self, cells_sim,cell_lines_sim_2,drugMat):
        self.dsMat = drugMat - np.diag(np.diag(drugMat))
        self.PSMat = cells_sim - np.diag(np.diag(cells_sim))
        self.PSMat_2 = cell_lines_sim_2 - np.diag(np.diag(cell_lines_sim_2))
        if self.K1 > 0:
